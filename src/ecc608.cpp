...
uint8_t ECC608Class::initializeHW()
{
	if (initialized)
	{
		return 0;
...
uint8_t ECC608Class::readProvisionItem(enum ecc_data_types type, uint8_t *buffer, uint8_t *length)
{
	...
	// Determine slot size
	if ((atca_status = atcab_get_zone_size(ATCA_ZONE_DATA, SLOT_NUM, &slot_size)) != ATCA_SUCCESS)
	{
		*length = 0;
		return atca_status;
	}

	// Traverse items in slot, return matching item in buffer, actual length in *length
	do
	{
		// Read data header from ECC slot
		if ((atca_status = atcab_read_bytes_zone(
				 ATCA_ZONE_DATA, SLOT_NUM, offset, h.bytes, sizeof(h))) != ATCA_SUCCESS)
		{
			*length = 0;
			return atca_status;
		}
		if (h.header.type == type)
		{
			...
			{ // make sure there is room for \0 terminator
				...
				if ((atca_status = atcab_read_bytes_zone(
						 ATCA_ZONE_DATA, SLOT_NUM, offset + sizeof(h), buffer, size)) != ATCA_SUCCESS)
				{
					*length = 0;
					return atca_status;
				}
